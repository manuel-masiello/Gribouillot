...

{
    ...
    undoAction->setIcon(QIcon(":/Resources/Icons/edit-undo.png"));
    QAction* redoAction = undoStack->createRedoAction(this, tr("&Redo"));
    redoAction->setShortcuts({QKeySequence::Redo, QKeySequence(Qt::CTRL | Qt::Key_Y)});
    redoAction->setIcon(QIcon(":/Resources/Icons/edit-redo.png"));

    QMenu* menuEdit = new QMenu(tr("Edit"), this);
    menuEdit->addAction(undoAction);
    menuEdit->addAction(redoAction);
    menuBar()->insertMenu(ui->menuOptions->menuAction(), menuEdit);

    //Also expose Undo/Redo as toolbar buttons, at the very start of the toolbar
    QAction* toolbarSeparator = ui->toolBar->insertSeparator(ui->toolBar->actions().first());
    ui->toolBar->insertAction(toolbarSeparator, redoAction);
    ui->toolBar->insertAction(redoAction, undoAction);
...
void Gribouillot::on_actionImport_layer_triggered()
{
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        undoStack->clear();//discard the AddItemCommands generated by loading the layer's items
...
void Gribouillot::blackWhiteTlBttClicked(bool isChecked)
{
    ...
    {/* Create a grayscale image */
         ...
         * range of pixel = a slice of the mapImage from start_x to end_x
         */
        auto color2bw = [&](int start_x, int end_x)
        {
            QColor oldColor;
        ...
        if (thrd_count > 1 && mapImage->width() > 200)
        {
            ...
            for (/*i=0*/; i < thrd_count -1; i++)
                tt[i] = std::thread(color2bw, i*chunk_size, (i+1)*chunk_size);
        ...
        color2bw(i*chunk_size, bwMapImage->width());
...
{
    undoStack->clear();//commands may reference the layer about to be destroyed
...
/**
 * @brief   push an undo command upon request from a layer
 */
void Gribouillot::doItemAdded(GribouillotLayer* layer, QGraphicsItem* item)
{
    undoStack->push(new AddItemCommand(layer, item));
}
